...
# 1. Inject wa_helper.js if not present
if 'wa_helper.js' not in text:
    text = text.replace('<script src="index.js"></script>', '<script src="wa_helper.js"></script>\n  <script src="index.js"></script>')

# 2. Fix cleanPhone issues
text = text.replace('const cleanPhone = (settings.whatsapp || "9647800000000").replace(/\D/g, "");', 'const cleanPhone = (settings.whatsapp || "9647800000000").replace(/\D/g, "").replace(/^0/, "964");')
text = text.replace('const targetPhone = (settings.roomWhatsapp || settings.whatsapp || "9647800000000").replace(/\D/g, "");', 'const targetPhone = (settings.roomWhatsapp || settings.whatsapp || "9647800000000").replace(/\D/g, "").replace(/^0/, "964");')

# 3. Replace the giftForm listener
old_handler = r"document\.getElementById\('giftForm'\)\?\.addEventListener\('submit',async event=>\{.*?refreshGiftPayment\(\);\n *\}\);"
...
text = re.sub(old_handler, new_handler.replace('\\', '\\\\'), text, flags=re.DOTALL)
