...
async function profile(auth) { const request = new Request('https://worker.test/api/loyalty/profile', { method: 'GET', headers: { Origin: env.ALLOWED_ORIGINS, Authorization: `Bearer ${auth}` } }); const response = await handleLoyaltyRoutes(request, env, new URL(request.url)); return { status: response.status, body: await response.json() }; }
...
const writesBeforeUnlinkedLogin = writes;
const unlinkedLogin = await login('101-5', '9753');
assert.equal(unlinkedLogin.status, 200);
const unlinkedUid = 'loyalty-member:101-5';
assert.equal(root.loyalty_customers['101-5'].uid, unlinkedUid);
assert.equal(root.loyalty_links[unlinkedUid], '101-5');
const unlinkedProfile = await profile(token(unlinkedUid, '101-5'));
assert.equal(unlinkedProfile.status, 200);
assert.equal(unlinkedProfile.body.profile.membershipNumber, '101-5');
assert.equal(unlinkedProfile.body.profile.currentHearts, 3);
assert.equal(writes - writesBeforeUnlinkedLogin, 3);
