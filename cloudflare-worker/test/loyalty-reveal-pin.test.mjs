...
const root = { admins: { 'admin-1': { role: 'super_admin', status: 'active' } }, loyalty_customers: { '101-1': { uid: 'member-1', currentHearts: 2 }, '101-2': { uid: 'member-2', currentHearts: 1 } }, loyalty_links: { 'member-1': '101-1', 'member-2': '101-2' }, loyalty_credentials: { '101-1': await createCredential('5678', env.LOYALTY_PIN_PEPPER), '101-2': await createCredential('2468', env.LOYALTY_PIN_PEPPER) } };
...
assert.equal(Object.values(root.loyalty_logs || {}).some(log => log.type === 'PIN_REVEALED' && !('pin' in log)), true);
