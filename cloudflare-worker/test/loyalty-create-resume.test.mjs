...
async function create(payload, auth = token('admin-1')) { const request = new Request('https://worker.test/api/admin/loyalty/create', { method: 'POST', headers: { Origin: env.ALLOWED_ORIGINS, Authorization: `Bearer ${auth}`, 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); const response = await handleLoyaltyRoutes(request, env, new URL(request.url)); return { status: response.status, allowOrigin: response.headers.get('Access-Control-Allow-Origin'), body: await response.json() }; }
try {
  ...
  const unavailable = await create({ requestId: 'create-firebase-unavailable', name: 'Unavailable customer', phone: '', hearts: 0, memberType: 'زبون' });
  assert.equal(unavailable.status, 503); assert.equal(unavailable.allowOrigin, env.ALLOWED_ORIGINS); assert.equal(unavailable.body.error, 'BACKEND_UNAVAILABLE'); assert.equal(Object.keys(root.loyalty_customers).length, 2);
