...
async function route(request, env, url) {
  ...
  try {
    ...
    const status = ['AUTH_REQUIRED', 'AUTH_INVALID'].includes(code) ? 401 : code === 'FORBIDDEN' ? 403 : ['CLUB_MEMBER_NOT_FOUND', 'NOT_FOUND', 'PROFILE_NOT_FOUND', 'SUB_REQUEST_NOT_FOUND'].includes(code) ? 404 : code === 'BACKEND_UNAVAILABLE' ? 503 : firebaseStatus === 401 || firebaseStatus === 403 ? 502 : firebaseStatus >= 500 ? 503 : ['BACKEND_AUTH_ERROR', 'INTERNAL_ERROR', 'PIN_REVEAL_WRITE_UNVERIFIED', 'CREDENTIAL_CREATE_FAILED', 'PIN_REVEAL_KEY_NOT_CONFIGURED', 'PROFILE_READBACK_FAILED'].includes(code) ? 500 : ['GIFT_ALREADY_REDEEMED', 'SUB_REQUEST_NOT_PENDING', 'SUB_CUSTOMER_ALREADY_ACTIVE', 'CONCURRENT_MODIFICATION', 'COUNTER_CONFLICT'].includes(code) ? 409 : code === 'INVALID_CONTENT_TYPE' ? 415 : code === 'PAYLOAD_TOO_LARGE' ? 413 : 400;
