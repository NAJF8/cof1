...
import { firebaseAdminRequest } from './firebase-admin.js';
...
function tableMinutes(value){const m=String(value||'').trim().match(/^(\d{1,2}):(\d{2})$/);return m&&Number(m[1])<24&&Number(m[2])<60?Number(m[1])*60+Number(m[2]):null;}
function tableInterval(startTime,endTime){const start=tableMinutes(startTime),rawEnd=tableMinutes(endTime);if(start==null||rawEnd==null)return null;const end=rawEnd<=start?rawEnd+1440:rawEnd;return end>start?{start,end}:null;}
function tableOverlap(a,b){return Boolean(a&&b&&[-1440,0,1440].some(shift=>a.start<b.end+shift&&b.start+shift<a.end));}
function baghdadDateParts(){const parts=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Baghdad',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hourCycle:'h23'}).formatToParts(new Date()).filter(x=>x.type!=='literal').reduce((o,x)=>(o[x.type]=x.value,o),{});return parts;}
function pastBaghdad(date,start){const p=baghdadDateParts();return `${date}T${start}`<`${p.year}-${p.month}-${p.day}T${p.hour}:${p.minute}`;}
async function handleTableAvailability(request,env){const cors=corsHeaders(request,env);if(request.method==='OPTIONS')return cors?new Response(null,{status:204,headers:cors}):json({ok:false,error:'ORIGIN_NOT_ALLOWED'},403);if(request.method!=='POST')return withCors(request,env,{ok:false,error:'METHOD_NOT_ALLOWED'},405,{Allow:'POST, OPTIONS'});if(!cors)return json({ok:false,error:'ORIGIN_NOT_ALLOWED'},403);let body;try{body=await request.json();}catch{return withCors(request,env,{ok:false,error:'INVALID_REQUEST'},400);}const eventId=String(body?.eventId||'').trim(),date=String(body?.date||'').trim(),startTime=String(body?.startTime||'').trim(),endTime=String(body?.endTime||'').trim(),requested=tableInterval(startTime,endTime);if(!/^[A-Za-z0-9_-]{1,120}$/.test(eventId)||!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(date)||!requested||pastBaghdad(date,startTime))return withCors(request,env,{ok:false,error:'INVALID_BOOKING_PERIOD'},400);try{const [event,all]=await Promise.all([firebaseAdminRequest(env,`events/${eventId}`),firebaseAdminRequest(env,'eventBookings')]);if(!event||event.isActive===false||['cancelled','disabled'].includes(String(event.status||'').toLowerCase()))return withCors(request,env,{ok:false,error:'EVENT_UNAVAILABLE'},409);const total=Math.max(1,Number(event.tableCount??event.capacity??30)||30),reserved=Object.values(all||{}).reduce((count,row)=>{if(!row||row.eventId!==eventId||row.date!==date||String(row.status||'').toLowerCase()!=='confirmed')return count;const existing=tableInterval(row.startTime||row.time,row.endTime);return tableOverlap(existing,requested)?count+Math.max(1,Number(row.tableCount)||1):count;},0);return withCors(request,env,{ok:true,total,reserved:Math.min(total,reserved),available:Math.max(0,total-reserved),timeZone:'Asia/Baghdad'});}catch(error){console.error('[TABLE_AVAILABILITY_FAILED]',{code:String(error?.message||'UNKNOWN').slice(0,80)});return withCors(request,env,{ok:false,error:'AVAILABILITY_UNAVAILABLE'},503);}}
let firebasePublicKeyCache = {expiresAt:0,keys:{}};

function workerBuildHeaders(env){const build=String(env.WORKER_BUILD||"").trim();return /^[0-9a-f]{40}$/i.test(build)?{"X-Worker-Build":build}:{};}
function json(body,status=200,headers={},env={}){return new Response(JSON.stringify(body),{status,headers:{"Content-Type":"application/json; charset=utf-8","Cache-Control":"no-store","X-Content-Type-Options":"nosniff",...workerBuildHeaders(env),...headers}});}
...
function withCors(request,env,body,status=200,headers={}){return json(body,status,{...(corsHeaders(request,env)||{}),...headers},env);}
...
function inferredProductTags(item,category){const text=String([item?.name,item?.nameAr,item?.nameEn,item?.description,item?.descriptionAr,item?.descriptionEn,category].filter(Boolean).join(" ")).toLowerCase(),tags=[];if(/قهوة|coffee|espresso|latte|لاتيه|امريكانو|موكا|كابتشينو|كولد برو|cold brew/.test(text))tags.push("coffee");if(/بارد|ايس|آيس|ثلج|cold|iced|juice|عصير|كركديه|refresh|منعش/.test(text))tags.push("cold");if(/حار|ساخن|hot/.test(text))tags.push("hot");if(/كركديه|منعش|refresh|عصير|juice|ليمون|مانجو|فراولة|فاكه|fruity/.test(text))tags.push("refreshing","non-coffee");if(/حلو|حالي|sweet|سكر|كراميل|شوكولات|dessert|حلى|كيك|براوني|كوكي/.test(text))tags.push("sweet");return normalizeTags([...tags,...(Array.isArray(item?.aiTags)?item.aiTags:[])]);}
function normalizePreferredProducts(raw){return Object.fromEntries(Object.entries(isPlainObject(raw)?raw:{}).map(([id,value])=>[cleanText(String(id),80),{enabled:value?.enabled===true,priority:Math.max(0,Math.min(100,Number(value?.priority)||0))}]).filter(([id,value])=>id&&value.enabled));}
...
function normalizeMenu(raw){const entries=Array.isArray(raw)?raw.map((item,index)=>[String(index),item]):Object.entries(isPlainObject(raw)?raw:{});return entries.filter(([,item])=>item&&!item.hidden&&!item.comingSoon&&item.available!==false&&item.active!==false).map(([key,item])=>{const category=cleanText(item.categoryId??item.category,80);return{productId:cleanText(String(item.id??key),80),name:localizedValue(item,"name"),nameAr:cleanText(item.nameAr,100),nameEn:cleanText(item.nameEn,100),category,description:localizedValue(item,"description"),price:Number.isFinite(Number(item.price))?Number(item.price):0,available:true,aiTags:inferredProductTags(item,category)};}).filter(item=>item.name).slice(0,160);}
...
function scoreRecommendations(menu,message,preferences,rules,preferredProducts={}){const detected=messageTags(message,preferences),matched=matchingRules(message,rules),preferred=new Set([...detected,...matched.flatMap(rule=>rule.preferredTags)]),avoid=new Set(matched.flatMap(rule=>rule.avoidTags)),direct=new Set(matched.flatMap(rule=>rule.preferredProducts));return menu.map(product=>{let score=0;for(const tag of product.aiTags){if(preferred.has(tag))score+=10;if(avoid.has(tag))score-=25;}if(direct.has(product.productId))score+=50;score+=matched.reduce((total,rule)=>total+(product.aiTags.some(tag=>rule.preferredTags.includes(tag))?rule.priority:0),0);const adminPreference=preferredProducts[product.productId];if(score>0&&adminPreference?.enabled)score+=Math.min(8,Number(adminPreference.priority)||0)*0.5;return{...product,score};}).filter(product=>product.score>0).sort((a,b)=>b.score-a.score||a.name.localeCompare(b.name,"ar")).slice(0,3);}
...
function localMenuReply(message,context,preferences){
    ...
    const picks=scoreRecommendations(menu,message,preferences,context.rules,context.config.preferredProducts).slice(0,3);
    if(picks.length)
...
function relevantMenuForAI(menu,message,preferences){const t=String(message||"").toLowerCase(),dessert=/حلى|حلوى|dessert|cake|كيك/.test(t)||preferences.type==="dessert",coffee=/قهوة|coffee|espresso|لاتيه|americano/.test(t)||preferences.type==="coffee",cold=/بارد|ثلج|ايس|آيس|cold|ice/.test(t)||preferences.temperature==="cold",noCoffee=/بدون قهوة|مو قهوة|لا اريد قهوة|non.?coffee|no coffee/.test(t),refreshing=/منعش|refresh|عصير|juice|كركديه|ليمون|fruit/.test(t);let list=menu;if(dessert)list=menu.filter(p=>p.category.includes("حلويات")||p.aiTags.includes("dessert"));else if(coffee&&!noCoffee)list=menu.filter(p=>p.aiTags.includes("coffee"));else if(noCoffee)list=menu.filter(p=>!p.aiTags.includes("coffee")&&(!cold||p.aiTags.includes("cold"))&&(!refreshing||p.aiTags.includes("refreshing")||p.aiTags.includes("fruity")));else if(cold)list=menu.filter(p=>p.aiTags.includes("cold"));return (list.length?list:(noCoffee?[]:menu)).slice(0,24).map(compactProduct);}
function compactPrompt(context,language,message,preferences,recommendations){
    ...
    return "You are the 101 COFFEE barista. Your current name is "+assistantName+". "
        ...
        +"\nPREFERRED PRODUCTS (soft priority only when they match the request): "+JSON.stringify(context.config.preferredProducts||{})
